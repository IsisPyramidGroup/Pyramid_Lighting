...
/*****************************************************************************
Electrical Test for strands of LEDs of two similar types:
	Adafruit strands used on Caroushell using the WS2801 chip
	Generic strands used on Starfish using the SM16716 chip

NOTE! These two LED strand types use DIFFERENT pin assignments on the connector.

Connect pixel strands to the SPI pins: 11 (DATA) and 13 (CLOCK)

Jumper pin 8 to pin 9 to test Caroushell strands.
Remove jumper to test Starfish strands.

*****************************************************************************/

#define  MAX_LEDS_PER_STRAND  50  // LEDs
#define	 LEDS_PER_WS2801_STRAND		50
#define	 LEDS_PER_SM16716_STRAND	19    // 18 plus one for the output connector test

#define  WIPE_WAIT        50  // ms
...
void loop() {
  ...
  // (re-) detect which kind of strand we're talking to (check jumper)
  digitalWrite(JUMPER_OUT, HIGH);
  in1 = digitalRead(JUMPER_IN);
  digitalWrite(JUMPER_OUT, LOW);
  in2 = digitalRead(JUMPER_IN);
  if (in1 == in2) {
    strand_type = WS2801;
    led_count = LEDS_PER_WS2801_STRAND;
  }
  else {
    strand_type = SM16716;
    led_count = LEDS_PER_SM16716_STRAND;
  }
